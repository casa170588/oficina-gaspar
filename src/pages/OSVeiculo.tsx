...
import { Camera, FileDown, Plus, Trash2, ImageIcon, Search, AlertTriangle } from "lucide-react";
...
const OSVeiculo = () => {
  ...
  const addPeca = (peca: ItemEstoque) => {
    if (peca.quantidade <= 0) {
      setAlertMsg("PEÇA DE RASTREAMENTO ESGOTADA!");
        ...
        {showEstoqueAlert && (
          ...
          >
            <div className="bg-card border-2 border-primary rounded-xl p-8 glow-green text-center max-w-sm">
              ...
              <p className="font-display text-xl font-bold text-primary mb-2">⚠ {alertMsg}</p>
